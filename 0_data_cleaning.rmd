...
<!-- # Using Borda Count: #1 gets max points, #2 gets max-1, etc. -->
<!-- max_rank <- length(entries_list)  # Total number of ranking positions (20) -->
<!-- for (i in 1:length(entries_list)) { -->
<!--   # Borda Count: rank 1 = 20 points, rank 2 = 19 points, ..., rank 20 = 1 point -->
<!--   borda_score <- (max_rank + 1) - i -->

<!--   # Add a new column 'borda_score' to the current dataframe -->
<!--   entries_list[[i]]$borda_score <- rep(borda_score, nrow(entries_list[[i]])) -->
<!-- } -->
...
<!-- ``` -->
<!-- Creating a Borda-weighted list -->
<!-- ```{r} -->


<!-- #Create a new list for sorting popular count using Borda scores -->
<!-- bordaWeightedReleasesList <- countedEntriesList -->

<!-- df <- bordaWeightedReleasesList %>% -->
<!--   group_by(TitletoLowercase) %>% -->
<!--   mutate(borda_sum = sum(borda_score)) %>% -->
<!--   ungroup() -->
