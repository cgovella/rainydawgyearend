...
# Create a new column 'toLower' in countedReleasesList with lowercase values from 'Title'
countedReleasesList$toLower <- tolower(countedReleasesList$Title)

library(dplyr)
library(stringdist)

countedReleasesList$CountOfMentions <- sapply(seq_along(countedReleasesList$toLower), function(i) sum(stringdist::stringdist(countedReleasesList$toLower[i], countedReleasesList$toLower) <= 2) - 1)
...
#Drop any rows that are incomplete
# #Loop through the list of dataframes
# for (i in 1:length(weightedEntriesList)) {
#   weightedEntriesList[[i]] <- weightedEntriesList[[i]] %>% 
#     filter(!(is.na(Artist) & is.na(Title)) & !(Artist == "" & Title == ""))
# }

# # Loop through the list of dataframes
# for (i in 1:length(weightedEntriesList)) {
#   # Convert weightedEntriesList[[i]] to a dataframe if it's not already one
#   df <- as.data.frame(weightedEntriesList[[i]])
#   
#   # Apply filter() function to remove rows where both 'Artist' and 'Title' columns are NA or blank
#   df <- df %>% 
#     filter(!(is.na(Artist) & is.na(Title)) & !(Artist == "" & Title == ""))
#   
#   # Replace my_list[[i]] with the filtered dataframe
#   weightedEntriesList[[i]] <- df
# }
# weightedEntriesList <- drop_na(countedEntriesList) 
