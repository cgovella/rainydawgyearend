...
# Load data
data <- read_excel("data/RainyDawg Radio End of 2025 Music List (Responses).xlsx")

# Drop the first row, it's a placeholder
#data <- data[-1, ]

# Unfortunately, the 10th row contains data that was input by error
# the entries are not from 2025. so we need to drop the 10th row:
data <- data[-9, ]

# Justin Bieber released 'Swag' and then a deluxe version of the same album with remixes
# so we need to drop the cell in the 3rd row, 6th column:
data[3, 6] <- NA

# Swap phrase order in cell [13, 5] from "A - B" to "B - A"
parts <- strsplit(as.character(data[13, 5]), " - ")[[1]]
data[13, 5] <- paste(parts[2], parts[1], sep = " - ")
parts <- NA

parts <- strsplit(as.character(data[13, 6]), " - ")[[1]]
data[13, 6] <- paste(parts[2], parts[1], sep = " - ")
parts <- NA

parts <- strsplit(as.character(data[13, 7]), " - ")[[1]]
data[13, 7] <- paste(parts[2], parts[1], sep = " - ")
parts <- NA

parts <- strsplit(as.character(data[12, 5]), " - ")[[1]]
data[12, 5] <- paste(parts[2], parts[1], sep = " - ")
parts <- NA


# now we should move the values one column to the left to save the votes
data[2, 6] <- data[2, 7]
data[2, 7] <- data[2, 8]
data[2, 8] <- data[2, 9]
data[2, 9] <- data[2, 10]
...
new_column_names <- c("Timestamp", "Email", "Name", "Role", "Rank01", "Rank02", "Rank03", "Rank04", "Rank05", "Rank06", "Rank07", "Rank08", "Rank09", "Rank10", "Rank11", "Rank12", "Notes")
...
max_rank <- 12  # Total number of ranking positions (Rank01 through Rank12)
...
cleaned_data <- long_data %>%
  ...
  mutate(
    ...
    Title = trimws(Title),
    # Borda Count: rank 1 = 12 points, rank 2 = 11 points, ..., rank 12 = 1 point
...
# Fix artist name spelling: "Pink Pantheress" -> "Pinkpantheress"
cleaned_data$Artist <- gsub("Pink Pantheress", "Pinkpantheress", cleaned_data$Artist)

# Fix artist name spelling: "FKA Twigs" -> "FKA twigs"
cleaned_data$Artist <- gsub("FKA Twigs", "FKA twigs", cleaned_data$Artist)
